...
PORT=4001
DATABASE_URL=postgresql://postgres:replace-with-a-strong-database-password@localhost:5432/parabellum_auth
...
# S3_ACCESS_KEY_ID=replace-with-your-s3-access-key
# S3_SECRET_ACCESS_KEY=replace-with-your-s3-secret-key
...
- ✅ **Templates de rôles prédéfinis** (ADMIN, EMPLOYEE, SERVICE_MANAGER, …)
- ✅ **Flux d'approbation** pour les changements sensibles via requêtes
- ✅ **Notifications** automatiques aux administrateurs et utilisateurs
...
- `POST /api/permission-requests` : soumettre un changement de permission
- `GET /api/permission-requests?status=PENDING` : lister les demandes en attente
- `PATCH /api/permission-requests/:id/approve` ou `/reject` pour valider ou refuser

La variable d'environnement `ENFORCE_PERMISSION_WORKFLOW=true` empêche les modifications directes sur `/api/permissions/roles`.
