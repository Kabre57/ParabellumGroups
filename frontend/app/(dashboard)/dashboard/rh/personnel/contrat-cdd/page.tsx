...
export default function ContratCddPage() {
  ...
  return (
    <>
      <LogipaieCrudPage
        ...
        description="Contrats CDD générés à partir des fiches du personnel (format LOGIPAIE)."
        queryKey={['logipaie-contrats-cdd']}
        queryFn={async () => {
          const response = await hrService.getContracts({ limit: 200 });
          const data = response.data.filter(
            (row: any) => (row.contractType || row.typeContrat || '').toUpperCase() === 'CDD'
          );
        ...
        mapRows={(rows) =>
          rows.map((row: any) => ({
            ...
            dateDebut: row.startDate
              ...
              : '-',
            dateFin: row.endDate ? new Date(row.endDate).toLocaleDateString('fr-FR') : '-',
            salaireBase: row.salary ?? row.salaireBaseMensuel ?? '-',
            statut: row.status ?? row.statutContrat ?? '-',
            raw: row,
          }))
        }
        emptyLabel="Aucun contrat CDD disponible."
      />
        ...
        <LogipaieDocumentModal
          ...
          sections={[
            ...
            },
            {
              ...
              rows: [
                { label: 'Type de contrat', value: 'CDD' },
                { label: "Date d'entrée", value: selected.dateDebut },
                { label: 'Date de fin prévue', value: selected.dateFin },
                ...
                { label: 'Motif', value: selected.raw?.motifRecrutement ?? '-' },
