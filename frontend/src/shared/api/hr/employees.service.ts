...
const mapEmployeeFromApi = (api: any): Employee => {
  ...
  return {
    ...
    cnamNumber: api.numeroCnam ?? api.cnamNumber ?? api.cnam_number,
    ...
    isActive: api.isActive ?? (status ? String(status).toUpperCase() === 'ACTIF' : true),
