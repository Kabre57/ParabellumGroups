...
export function FinancialDashboard() {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => analyticsService.getFinanceStats(),
  ...
  const finance = data;
  const revenueVariation = finance.revenus?.variation ?? 0;
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">
                  {new Intl.NumberFormat('fr-FR', {
                    style: 'currency',
                    currency: 'XOF',
                  }).format(finance.revenus?.total ?? 0)}
                </p>
                <div className={`flex items-center gap-1 mt-1 text-sm ${
                  revenueVariation > 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  <TrendingUp className={`h-4 w-4 ${revenueVariation < 0 ? 'transform rotate-180' : ''}`} />
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">
                  {new Intl.NumberFormat('fr-FR', {
                    style: 'currency',
                    currency: 'XOF',
                  }).format(finance.depenses?.total ?? 0)}
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">
                  {new Intl.NumberFormat('fr-FR', {
                    style: 'currency',
                    currency: 'XOF',
                  }).format(finance.tresorerie?.disponible ?? 0)}
                </p>
              </div>
            </div>
          </CardContent>
