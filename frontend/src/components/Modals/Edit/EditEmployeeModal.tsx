...
export const EditEmployeeModal: React.FC<EditEmployeeModalProps> = ({ isOpen, onClose, employee }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      nationality: employee?.nationality || 'Ivoirienne',
  ...
  const updateEmployeeMutation = useMutation({
    mutationFn: (data: EditEmployeeFormData) => employeeService.update(employee.id, data),
    onSuccess: () => {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900 flex items-center">
            ...
            Modifier l'Employé {employee.employeeNumber}
          </h3>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {servicesList.map((service: any) => (
                      <option key={service.id} value={service.id} selected={service.id === employee.serviceId}>
                        {service.name}
                      </option>
                    ))}
                  </select>
