...
export const EditLeaveModal: React.FC<EditLeaveModalProps> = ({ 
  ...
  const updateLeaveMutation = useMutation({
    mutationFn: (data: EditLeaveFormData) => leaveService.update(leave.id, data),
    onSuccess: () => {
  ...
  const onSubmit = async (data: EditLeaveFormData) => {
    setIsLoading(true);
    try {
      await updateLeaveMutation.mutateAsync(data);
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900 flex items-center">
            ...
            Modifier la Demande de Congé {leave.leaveNumber}
          </h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4 space-y-6">
            ...
            <div className="relative">
              ...
              >
                ...
                {employeesList.map((employee: any) => (
                  <option key={employee.id} value={employee.id} selected={employee.id === leave.employeeId}>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            >
              {isLoading ? 'Modification...' : 'Modifier la demande'}
            </button>
          </div>
        </form>
      </div>
