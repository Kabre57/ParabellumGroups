...
import { adminPermissionsService } from '@/shared/api/admin/admin.service';
...
export const CreatePermissionModal: React.FC<CreatePermissionModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const onSubmit = async (data: CreatePermissionFormData) => {
    try {
      await adminPermissionsService.createPermission(data);
      toast.success('Permission creee avec succes');
      reset();
      onSuccess?.();
      onClose();
    } catch (error: any) {
      console.error('Erreur creation permission:', error);
      toast.error(error?.message || 'Erreur lors de la creation de la permission');
    }
  };
